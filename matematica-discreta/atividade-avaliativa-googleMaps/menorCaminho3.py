...
def encontrar_menor_caminho(G, origem, destino, modo):
    ...
    while fila:
        ...
        if no not in visitados:
            vizinhos = G[no]
            menor_peso = float('inf')  # Inicialize com infinito

            for vizinho in vizinhos:
                peso_aresta = G[no][vizinho]['weight']
                if modo == 'carro' and peso_aresta != 1:
                    continue
                if modo == 'a_pe' and peso_aresta == 1:
                    continue
            ...
            if proximo_no == destino:
                return caminho + [proximo_no]
            else:
                fila.append((proximo_no, caminho + [proximo_no]))
